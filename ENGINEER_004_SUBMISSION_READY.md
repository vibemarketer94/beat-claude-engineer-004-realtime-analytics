...
**Zero-loss posture.** Durable intake, at-least-once processing, idempotent writes, DLQs, Kinesis/S3 replay, and parity checks. Backpressure degrades customer-visible freshness *before* dropping accepted events. Noisy tenants get quotas and isolated partitions; high-volume tenants can move to dedicated streams or tables.
...
- **Month 3 [Observed] — MVP cutover for a cohort.** Flink dedupe/sessionization, DynamoDB hot aggregates, under-5-second [Observed] dashboards, dual-write, and the parity/freshness gates. Outcome: real-time dashboards live for an internal + design-partner cohort on the new path, with tenant rollback.
